...
fn writeGrammar(grammar: Grammar) !void {
    ...
    // Actions
    {
        ...
        try out.stream.write(
            \\// AutoGenerated file
            \\
            \\pub fn reduce_actions(comptime Parser: type, parser: *Parser, rule: usize) !?*c_void {
            \\    const stack_ = parser.stack;
            \\    const items_ = parser.stack.items;
            \\    const top_ = parser.stack.len - 1;
            \\
            \\    switch(rule) {
            \\
        );
        ...
        while(i < grammar.productions.len) : (i += 1) {
            ...
            while(ti < production.symbol_types.len) : (ti += 1) {
                ...
                try out.stream.print(" " ** 12 ++ "const arg{} = @bitCast(?*{}, items_[top_ - {}].value){};\n", ti+1, symbol_type.name, production.symbol_types.len - ti - 1, if(symbol_type.optional) "" else ".?");
            }
            try out.stream.write("\n");
            if(production.symbol_types.len > 0) {
                try out.stream.print(" " ** 12 ++ "// Readjust the parse stack\n");
                try out.stream.print(" " ** 12 ++ "stack_.len -= {};\n\n", production.symbol_types.len);
            }
            if(production.body.len > 0) {
                ...
                try out.stream.print(" " ** 12 ++ "// Return the result of the reduce action\n");
            try out.stream.write(" " ** 12 ++ "return @bitCast(?*c_void, result);\n");
            try out.stream.write(" " ** 8 ++ "},\n");
        }
        try out.stream.write(" " ** 8 ++ "else => unreachable,\n");
        try out.stream.write("    }\n");
        try out.stream.write("    return error.ParserError;\n");
        try out.stream.write("}\n");
    }
